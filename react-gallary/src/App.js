...
class App extends Component {
  ...
  Results = (query = "animals") => {
    fetch(
      ...
      .then((responseData) => {
        this.setState({ photos: responseData.photos.photo, tags: query });
      })
      .catch((err) => {
  ...
  // used link for each types of images
  cats = () => {
    fetch(
  ...
  birds = () => {
    fetch(
      `https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=${apiKey}&tags=birds&per_page=24&format=json&nojsoncallback=1`
    )
      .then((response) => response.json())
      .then((responseData) => {
